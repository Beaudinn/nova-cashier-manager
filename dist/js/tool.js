...
var render = function() {
  ...
  return _c(
    ...
    [
      _c(
        "loading-card",
        { staticClass: "mb-6 py-3 px-6", attrs: { loading: _vm.loading } },
        [
          !_vm.subscription
                  ...
                  _c("p", { staticClass: "text-90" }, [
                    _vm._v(
                      "\n                " +
                        ...
                        "\n                (" +
                        _vm._s(_vm.subscription.plan_amount.amount / 100) +
                        " " +
                        _vm._s(_vm.subscription.plan_currency.currency) +
                        " / " +
    ...
    methods: {
        ...
        loadUserData: function loadUserData() {
            ...
            axios.get('/nova-cashier-tool-api/user/' + this.userId).then(function (response) {
                ...
                _this.newPlan = response.data.subscription ? response.data.subscription.plan : null;
                console.log(_this.newPlan);
                _this.loading = false;
            });
        },
                            ...
                            [
                              _vm._v(
                                "\n                        " +
                                  _vm._s(plan.name) +
                                  " (" +
                                  _vm._s(plan.amount.amount / 100) +
                                  " " +
                                  _vm._s(plan.amount.currency) +
                                  " / " +
                  ...
                  _c("div", { staticClass: "w-3/4 py-4" }, [
                    _c("p", { staticClass: "text-90" }, [
                      _vm._v(
                        _vm._s(_vm.subscription.plan_amount.amount / 100) +
                          " (" +
                          _vm._s(_vm.subscription.plan_amount.currency) +
                          ") / " +
                          _vm._s(_vm.subscription.plan_interval)
                      )
                    ])
